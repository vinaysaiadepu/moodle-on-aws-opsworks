...
if [ $SIZE -gt <%= @moodledata_size %> ]; then
	...
	echo "$(date +"%s")" > /mnt/nfs/moodledata/filedir/start
	...
	if [ $? -eq 0 ]; then
		...
		echo "$(date +"%s")" > /mnt/nfs/moodledata/filedir/end
		aws s3 cp /mnt/nfs/moodledata/filedir/end s3://<%= @backup_bucket %>/<%= @stack %>/$1/moodledata/filedir/end --acl public-read
	else
